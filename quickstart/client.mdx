...
在本教程中，您将学习如何构建一个与 MCP 服务器连接的 LLM 驱动的聊天机器人客户端。建议您先完成[服务器快速入门](/quickstart/server)，该指南将带您完成构建第一个服务器的基本步骤。
...
首先，设置我们的导入并在 `index.ts` 中创建基本的客户端类：
