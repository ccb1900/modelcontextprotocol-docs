---
title: "工具"
description: "通过你的服务器让LLM执行操作"
---

工具是模型上下文协议（MCP）中的一项强大原语，允许服务器向客户端暴露可执行的功能。通过工具，LLM可以与外部系统交互、执行计算并在现实世界中采取行动。

<Note>
  工具设计为**模型控制**，意味着服务器向客户端暴露工具的目的是让AI模型能够自动调用它们（在人工监督下授予批准）。
</Note>
...
与[资源](/docs/concepts/resources)类似，工具由唯一的名称标识，并且可以包含描述以指导其使用。然而，与资源不同，工具代表可以修改状态或与外部系统交互的动态操作。
